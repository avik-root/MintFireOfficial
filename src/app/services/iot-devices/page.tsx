...
import { Smartphone, AlertTriangle, PackageSearch, Sparkles, Loader2 } from 'lucide-react';

export default function IotDevicesPage() {
  const [productsToDisplay, setProductsToDisplay] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [errorLoading, setErrorLoading] = useState<string | null>(null);
  const [sectionTitle, setSectionTitle] = useState("Related IoT Products");
  const [SectionIcon, setSectionIcon] = useState<React.ElementType>(() => Smartphone);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let currentTitle = "Related IoT Products";
      let currentIcon: React.ElementType = Smartphone;
